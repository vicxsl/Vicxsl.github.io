...
1. ==是判断两个变量或实例是不是指向同一个内存空间，equals是判断两个变量或实例所指向的内存空间的值是不是相同 `值相同`
...
![image-20220617180207368](https://s2.loli.net/2022/06/17/n3AUyNSYuIMlPwv.png)

2. 双向链表:除了包含单链表的部分,还增加的pre前一个节点的指针

![image-20220617180246921](https://s2.loli.net/2022/06/17/GENS2UCbPRZiMo9.png)
...
1. 需要注意Jdk 1.8中对HashMap的实现做了优化，当链表中的节点数据超过八个之后，该链表会转为红黑树来提高查询效率，从原来的O(n)到O(logn)

### HashMap在JDK1.7和JDK1.8中有哪些不同？HashMap的底层实现

在Java中，保存数据有两种比较简单的数据结构：数组和链表。数组的特点是：寻址容易，插入和删除困难；链表的特点是：寻址困难，但插入和删除容易；所以我们将**数组和链表结合在一起，发挥两者各自的优势**，使用一种叫做拉链法的方式可以解决哈希冲突。

> 拉链法的数据结构很形象

#### HashMap JDK1.8之前

JDK1.8之前采用的是拉链法。拉链法：将链表和数组相结合。**也就是说创建一个链表数组**，数组中每一格就是一个链表。若遇到哈希冲突，则将冲突的值加到链中即可。

![image-20220617181159573](https://s2.loli.net/2022/06/17/rnV7ChIEDc6e89o.png)

#### HashMap JDK1.8之后

相比于之前的版本，jdk1.8在解决哈希冲突时有了较大的变化，当链表长度大于阈值（默认为8）时，将链表转化为红黑树，以减少搜索时间。

![image-20220617181234980](https://s2.loli.net/2022/06/17/cK1BY9wVFveXDoQ.png)
...
- 每个结点到叶子结点NILL所经过的黑色结点的个数一样的。[确保没有一条路径会比其他路径长出俩倍，所以红黑树是相对接近平衡的二叉树的！]
...
![image-20220620201620869](https://s2.loli.net/2022/06/20/Ck1rGpF9YSywcBx.png)
...
- 所以我们将数组和链表结合在一起，发挥两者各自的优势，就可以使用俩种方式：链地址法和开放地址法可以解决哈希冲突：
  ...
  ![image-20220620204017419](https://s2.loli.net/2022/06/20/pWJubUewAPcj2vF.png)
...
- 那怎么解决呢？
  1. HashMap自己实现了自己的hash() 方法，通过两次扰动使得它自己的哈希值高低位自行进
  行异或运算，降低哈希碰撞概率也使得数据分布更平均；
  2. 在保证数组长度为2的幂次方的时候，使用hash() 运算之后的值与运算（&）（数组长度 -
...
![image-20220620211030948](https://s2.loli.net/2022/06/20/YVeMJtxaEkWZKHF.png)

2. **JDK1.7的ConcurrentHashMap：**

![image-20220620211129653](https://s2.loli.net/2022/06/20/BrKlvgHzkY32hMI.png)

3. **JDK1.8的ConcurrentHashMap（TreeBin: 红黑二叉树节点 Node: 链表节点）：**

![image-20220620211207240](https://s2.loli.net/2022/06/20/T3hK5OBZHVWukpF.png)
...
![image-20220620211342160](https://s2.loli.net/2022/06/20/Bd1UVTfyQreXaqL.png)
...
![image-20220620211506654](https://s2.loli.net/2022/06/20/uFYRNQwx1g3rqyP.png)
